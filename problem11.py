...
grid_len = len(digit_list)

max_prod = 0
max_coord = ()
for i in range(grid_len):
    for j in range(grid_len):
        right_prod = down_prod = diag_prod_0 = diag_prod_1 = 0
        # right
        if j <= grid_len - GROUP_LEN:
            right_prod = reduce(lambda x, y: x * y, digit_list[i][j:j+GROUP_LEN])
        # down
        if i <= grid_len - GROUP_LEN:
            down_prod = reduce(lambda x, y: x * y, [line[j] for line in digit_list[i:i+GROUP_LEN]])
        # \
        if j <= grid_len - GROUP_LEN and i <= grid_len - GROUP_LEN:
            diag_prod_0 = reduce(lambda x, y: x * y, [digit_list[i+x][j+x] for x in range(GROUP_LEN)])
        # /
        if j >= GROUP_LEN - 1 and i <= grid_len - GROUP_LEN:
            diag_prod_1 = reduce(lambda x, y: x * y, [digit_list[i+x][j-x] for x in range(GROUP_LEN)])
        pmax = max(right_prod, down_prod, diag_prod_0, diag_prod_1)
        if pmax >= max_prod:
            max_prod = pmax
            max_coord = (i, j)
